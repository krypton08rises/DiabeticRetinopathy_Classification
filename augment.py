...
Folder_name="./augmentedImages/aug/"
Extension=".jpeg"
...
if __name__ == "__main__" :
    ...
    for index, row in df.iterrows():
        ...
        if row['level']>2:
            ...
            flip_image(image, 0, row['image'])  # horizontal
            flip_image(image, 1, row['image'])  # vertical
            flip_image( image, -1, row['image'])  # both
            add_light(row['image'], image, 1.2)
            add_light(row['image'], image, 0.8)
